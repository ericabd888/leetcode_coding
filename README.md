...
string ArrayMatching(string strArr[], int arrLength) {
  ...
  int small_idx = strArr[0].size() <= strArr[1].size()?0:1;
  for(int i = 0; i < strArr[small_idx].size(); i++){
    if(strArr[small_idx][i]!=','&&strArr[small_idx][i]!=' '&&strArr[small_idx][i]!='['&&strArr[small_idx][i]!=']'){
      v1 += strArr[small_idx][i];
    }
  }
  for(int i = 0; i < strArr[1-small_idx].size(); i++){
    if(strArr[1-small_idx][i]!=','&&strArr[1-small_idx][i]!=' '&&strArr[1-small_idx][i]!='['&&strArr[1-small_idx][i]!=']'){
      v2 += strArr[1-small_idx][i];
    }
  }
  int val1, val2, temp ;
  int count_now = 0;
  ...
  for(int i = count_now; i < v2.size(); i++){
    ans += v2[i];
    ans += '-';
  }
  string ans2;
...
class Main {
  ...
  public static String LongestWord(String sen) {
    ...
    Matcher m = r.matcher(sen);
    int max_one = Integer.MIN_VALUE;
    String max_string = "";
    while(m.find()){
      if(max_one < m.group().length()){
        max_string = m.group();
        max_one = m.group().length();
      }
...
class Main {
  ...
  public static String CharacterRemoval(String[] strArr) {
    ...
    // System.out.println(strArr[1].getClass().getName());
    List<String> list = new ArrayList<String>();
    String temp = "";
    String result = "";
    for(int i = 0; i < strArr[1].length(); i++){
      if(strArr[1].charAt(i)!=','){
        temp += strArr[1].charAt(i);
      }
      else {
        list.add(temp);
        temp = "";
    ...
    String X, Y;
    X = strArr[0];
    int mini_n_c = Integer.MAX_VALUE;
    for(int i = 0; i < list.size(); i++){
      Y = list.get(i);
      result = lcs(X,Y,X.length(),Y.length());
      if(list.contains(result)){
        if(mini_n_c >= (X.length()-result.length())){
          mini_n_c = (X.length()-result.length());
        }
          
      }

    }
    return String.valueOf(mini_n_c);
  }

  public static void main (String[] args) {  
    // keep this function call here     
    Scanner s = new Scanner(System.in);
    System.out.print(CharacterRemoval(s.nextLine())); 
  }
}
```
...
class Main {
  ...
  public static String EquivalentKeypresses(String[] strArr) {
    ...
    Matcher m1 = r.matcher(strArr[0]);
    Matcher m2 = r.matcher(strArr[1]);
    while(m1.find()){
      l1.add(m1.group());
    }
    while(m2.find()){
  ...
  public static void main (String[] args) {  
    // keep this function call here     
    Scanner s = new Scanner(System.in);
    System.out.print(EquivalentKeypresses(s.nextLine())); 
  }
}
```
